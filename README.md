...
Now when you insert a record, `seq` auto-increments and the trigger automatically sets `id = feistel_cipher(seq)`:
...
```sql
-- Encryption: seq → id
id = feistel_cipher(seq, bits, key)

-- Decryption: id → seq (using the same function!)
seq = feistel_cipher(id, bits, key)
```
